...
当前版本号： 0.6.4 （带api版）
本版本更新内容：
    根据VOCALOID软件特征更新了即时打开vsqx的逻辑
    增加延时的比例，减少停顿的比例
    把连去变声转移到mark阶段
bugs:
    vsqx的时钟频率是480帧/拍，程序中认为每帧对应1ms，是错误的
叹号问号等
    问号（疑问句）目的语气 √
    感叹句目的语气
    祈使
...
try:
    from reliance.tyTalk.talkerReliance.baiduCleint import client
    BAIDU_AVALIABLE = True
except ImportError:
    BAIDU_AVALIABLE = False
    client = lambda x: None
try:
    from reliance.tyTalk.talkerReliance import pyCorrect
    PY_CORR = pyCorrect.inf
except ImportError:
    pyCorrect = None
    PY_CORR = []
from reliance.tyTalk.talkerReliance import arabToChi
from reliance.tyTalk.talkerReliance import vocalDict
...
# 摘自autoVtalker，用于调用汉宁窗
from numpy import array, hanning, convolve

# import numpy.random.common
# import numpy.random.bounded_integers
# import numpy.random.entropy  # 用于提示pyinstaller
try:
...
class Talker:
    def __init__(self, online: bool=False):
        self.text = []
        self.mark_list = []
        self.note_list = []  # note:[py, start, length, pitch, intns]
        self.total_time = 0
        if BAIDU_AVALIABLE:
    ...
    def load(self, ipt='', speed=1.0, cmd='type', _vocalized=True, pyc=None):  # text to phonemes
        """
        ...
        text = text.strip()
        print(text)
        text = arabToChi.convert(text, simple=True)
        # fil = re.compile(
        ...
        self.text = text
        self.mark(parser, pyc_in=pyc)
        if _vocalized:
            self.vocalize(speed=speed)

    def mark(self, parser, pyc_in=None):
        # 引入数据，处理高级表示成为符号表示，此后各个音节独立拥有自己的属性，不再依赖临近文字
        """处理说明：
        利用pypinyin转化为拼音+标调
        停顿转化为名为‘-’的音节
        分句
        ...
        结构：text --> [each model (内部信息互通) --> each 字符序列 containing tuples of attributes] --> 字符序列attributes合并
        """
        text = self.text
        ...
        parse_marks = []  # [(词内序号1起始，长度，词性，词序号id0起始，字符本身)]
        words = []
        if parser == 'jieba':
            word_key = 0
            for x in jieba.posseg.cut(text):
                ...
                for ch in word:
                    no += 1
                    parse_marks.append((no, ln, flag, word_key, ch))
                words.append(word)
                word_key += 1
        elif parser == 'baidu':
        ...
        if ER:
            ...
            er_inf = [False for x in py_output]

        # 自动标注 “一”、“不”变调以及连去变调（覆盖了下面音高渲染部分的对应功能）
        # “一”规则过于宽泛，没有限制“一”单独成词的情况，如“不一而足”，但“一”在词尾的情况已经排除
        # ☆标记需要进行变调的“一”
        word_first_chr_index = 0
        ...
        for word in words:
            ...
            for i, c in enumerate(word):
                if c == '☆' and py_output[word_first_chr_index + i] == 'yi1':
                    if py_output[word_first_chr_index + i + 1][-1] in '123':
                        py_output[word_first_chr_index + i] = 'yi4'
                    elif py_output[word_first_chr_index + i + 1][-1] in '4aoeiuvngr':
                        py_output[word_first_chr_index + i] = 'yi2'
                if c == '不' and py_output[word_first_chr_index + i] == 'bu4':
                    try:
                        if py_output[word_first_chr_index + i + 1][-1] == '4':  # 同时排除后面是符号的情况
                            py_output[word_first_chr_index + i] = 'bu2'
                    except IndexError:
                        pass
                if py_output[word_first_chr_index + i][-1] == '3' and word_first_chr_index+i != 0 and py_output[word_first_chr_index + i -1][-1] == '3':
                    py_output[word_first_chr_index + i - 1] = py_output[word_first_chr_index + i - 1][:-1] + '2'
            word_first_chr_index += len(word)
        ...
        for i, word in enumerate(words):
            if len(word) == 2 and word[0] == word[1] and parse_marks[word_first_chr_index][2] == 'n':  # 名叠词后轻声
                modify.append((word_first_chr_index+1, ''.join([x for x in py_output[word_first_chr_index+1] if x not in '1234']), None))
            for i_w, chrc in enumerate(word):
                if chrc in '种中' and word not in ('中断', '集中'):
                    flag = parse_marks[i_w+word_first_chr_index][2]
                    if flag == 'v':
                        modify.append((i_w + word_first_chr_index, 'zhong4', None))
                if chrc == '得':
                    flag = parse_marks[i_w+word_first_chr_index][2]

                    dei = False
                    try:
                        past_word_last_chr_idx = word_first_chr_index - 1
                        past_mark = parse_marks[past_word_last_chr_idx]
                        if past_mark[2] in ('n', 'r', 'nz'):
                            dei = True
                    ...
                    if flag in ('u', 'ud'):
                        if dei:
                            modify.append((i_w + word_first_chr_index, 'dei3', None))
                        else:
                            modify.append((i_w + word_first_chr_index, 'de', None))
                if chrc == '处' and len(word) <= 2:
                    flag = parse_marks[i_w + word_first_chr_index][2]
                    if flag == 'v':
                        modify.append((i_w + word_first_chr_index, 'chu3', None))
                if chrc == '长' and len(word) == 1:
                    flag = parse_marks[i_w + word_first_chr_index][2]
                    if flag == 'a':
                        modify.append((i_w + word_first_chr_index, 'chang2', None))
                if chrc == '忒' and len(word) == 1:
                    flag = parse_marks[i_w + word_first_chr_index][2]
                    if flag == 'd':
                        modify.append((i_w + word_first_chr_index, 'tei1', None))
                if chrc == '咯' and len(word) == 1:
                    match = False
                    flag = parse_marks[i_w + word_first_chr_index][2]
                    if flag != 'v':
                        match = True
                    elif i_w + word_first_chr_index == len(text) - 1: # 在整个文本的最末
                        match = True
                    else:
                        try:
                            if re.match(SENT_ENDINGS, text[i_w + word_first_chr_index + 1]):  # 在句子结尾
                                match = True
                        except Exception as e:
                            print(e.__cause__)
                            print('"咯"拼音修正中出现错误')
                    if match:
                        modify.append((i_w + word_first_chr_index, 'lo', None))
            for item in rule:
                if item[0] in word:
                    for inf in item[1]:
                        if inf[2]:
                            if len(item[0]) > 1:
                                modify.append(((word.index(item[0]) + inf[0] + word_first_chr_index), inf[1], inf[2]))
                            else:
                                modify.append(((word.index(item[0]) + inf[0] + word_first_chr_index), inf[1], None))
                                print('有儿化规则不合理')
                        else:
                            modify.append(((word.index(item[0]) + inf[0] + word_first_chr_index), inf[1], inf[2]))
            word_first_chr_index += len(word)
        ...
        for pyc in pyc_in:
            if len(pyc) == 2:
                pyc = (pyc[0], pyc[1], False)
            if pyc[1][-1] == '?':  # ？
                continue
            if pyc[1][-1] == '0':
                pyc = (pyc[0], pyc[1][:-1], pyc[2])
            if pyc[1] == 'sil':
                pyc = (pyc[0], (text_d[pyc[0]],), pyc[2])
            modify.append(pyc)
        for mod in modify:  # 拼音修改
            if mod[1]:
                py_output[mod[0]] = mod[1]
        if ER:
            del_list = []
            for mod in modify:  # 儿化修改
                if mod[2]:
        ...
        for i, word in enumerate(words):
            ...
            word_first_chr_index += len(word)
            if re.match(SENT_ENDINGS, word) or '\n' in word or i + 1 == l:
                try:  # 主要依靠标点
                    ...
                    pass
                for i in range(word_first_chr_index):
                    sent_marks.append((i + 1, word_first_chr_index - 1, sent_goal))
                word_first_chr_index = 0
        ...
        for i in range(len(parse_marks)):
            self.mark_list.append({'py': py_marks[i], 'par': parse_marks[i], 'result_sent': sent_marks[i], 'proso': proso_marks[i]})

    def vocalize(self, speed=1.0):
        # 从符号成为波形
        """算法说明：
        分四声，轻声视为四声
        逗号处理为小停顿，句号处理为大停顿
        ...
        # 对时长渲染，以单字为单位循环的架构有点问题，和其他架构并不平行
        ...
        disc_per = 0  # 不连续性时间占原始时间长度（每个语法词后面都会有）
        disc_per_analyzed = 0.5
        prolong_per = 0.7  # 不连续性时间中，分配给延时的长度比例
        ...
        for i, chr_tempo in enumerate(chr_tempos):
            ...
            if py[0] == 'sil':
                ...
                if mark['result_sent'][0] == mark['result_sent'][1]:
                    length = length * sent_prolong

            # 临时插入基于句法分析的停顿（韵律词组的前身依据）
            if mark['par'][0] == mark['par'][1]:  # 若此字为语法词的最后一个字，则有停顿，否则没有
                disc = length * disc_per
            else:
                disc = 0
            if mark['proso'][1][2] and mark['proso'][1][0] is not None:
                disc = length * disc_per_analyzed * (1-prolong_per)
                disc *= (mark['proso'][1][0] / mark['proso'][1][1])
                length += disc*(prolong_per)
                disc *= 1-prolong_per
        ...
        def to_point_track():
            ...
            def tone_constrain(ori, l=2):
                limit = l  # 倍原有range
                return (1 / (1 + math.exp(-ori * extreme_tone / (limit * tone_range))) - 0.5) * limit * tone_range

            # 全局参数列表

            std_tone = 0
            std_tone_range = 3000
            pre_per = 0.1  # preparation time percent
            fastest_change = 15
            preset_per = 0.7  # 为后一音节的相对特征做准备的开始时间
            preset_aim = 0.5  # 为后做准备的程度 局部绝对值，需要改成全局相对值
            preset_aim_per = 0.8  # 适应后面发音的程度，调高会使波动变平
            sent_downward_per = 0.15  # 语调下倾程度
            extreme_tone = 1.2
            ...
            for idx, mark in enumerate(self.mark_list):
                ...
                try:
                    ...
                    if self.mark_list[idx - 1]['result_sent'][0] < self.mark_list[idx - 1]['result_sent'][1]:
                        point_list.append(before[-1])
                    slope = (before[-1][1] - std_tone * basic_points[0][1]) / pre_length  # pit/ms
                    if abs(slope) > fastest_change and mark['result_sent'][0] != 1:  # 留在那个高度
                ...
                bt_ctrl.append(- sent_downward_per * tone_range * (mark['result_sent'][0] - 1))


                # 基于句子结构的语调修改，同时改变了base_tone和range
                tr_ctrl = []
                sent_dist = mark['result_sent'][1] - mark['result_sent'][0]
                if mark['result_sent'][2] == 'qst' and -1 < sent_dist < goal_cover:
                ...
                try:
                    ...
                    exit()
                # 短停顿级音高重置，参考韵律短语重置程度
                if mark['py'][0] == 'sil' and mark['result_sent'][0] <= mark['result_sent'][1] and base_tone < std_tone:
        ...
        for i, chrc in enumerate(trck):
            ...
            for ind, point in enumerate(chrc[:-1]):
                ...
                for s in range(step_num):
                    point_list.append((start_time + step * s, start_tone + tone_step * s))
            seperate_list.append(point_list)
        total_tone = []
        total_time = []
        for points in seperate_list:
            for point in points:
                total_tone.append(point[1])
                total_time.append(point[0])
        pits = array(total_tone)
        N_hanning = 10
        hn_weights = hanning(N_hanning)
        pits_s = convolve(hn_weights / hn_weights.sum(), pits)[N_hanning - 1:-N_hanning + 1]
        pits = pits_s.tolist()
        pits = [pits[0]] * (N_hanning-1) + pits  # 补全长度
        # print('length:', len(pits), len(total_time))
        cur_ind = 0
        for i, points in enumerate(seperate_list):
            new_point_list = []
            for j in points:
                new_point_list.append((total_time[cur_ind], pits[cur_ind]))
                cur_ind += 1
            self.note_list[i].append(new_point_list)
    ...
    def to_vsqx(self, addr='', start=False):  # phonemes to vsqx
        tempo = reliance.global_vars.tempo
        ms_per_dot = reliance.global_vars.ms_per_dot

        note_list = self.note_list[:]
        pre = round(1000 / ms_per_dot)
        total_time = round((self.total_time + 100 + pre) / ms_per_dot)
        f0_level = 61
        pbs = 18
        ...
        for note in note_list[:]:
            ...
            for pit in note[3]:
                ...
                if p < -8190:
                    p = -8190
                text_insert += '\r\t\t\t<cc><t>{}</t><v id="P">{}</v></cc>'.format(
                    round((pit[0] + pre) / ms_per_dot), round(p))
            for intns in note[4]:
                if 'ang' in note[0]:
                    intns = (intns[0], intns[1] * 0.6)
                text_insert += '\r\t\t\t<cc><t>{}</t><v id="D">{}</v></cc>'.format(
                    round((intns[0] + pre) / ms_per_dot),
                    round(intns[1] * 64))
...
\t\t\t\t\t<v id="vibType">0</v>
\t\t\t\t</nStyle>
\t\t\t</note>""".format(start=round((note[1] + pre) / ms_per_dot), dur=round(note[2] / ms_per_dot) - 1, f0=f0_level,
                        py=py,
                        ph=dict_ph + er_ph + nasal_ph, vel=vel)
...
\t</aux>
        ...
        if start:  # vocaloid 无法重复打开同一个文件名的文件，所以用了点小技巧
            try:
                with open('output.vsqx'):
                    pass
                os.remove('output.vsqx')
            except FileNotFoundError:
                pass
            try:
                with open('output_A.vsqx'):
                    pass
                os.remove('output_A.vsqx')
                with open('output_B.vsqx', 'w', encoding='utf-8') as f:
                    f.write(text)
                os.startfile('output_B.vsqx')
            except FileNotFoundError:
                try:
                    with open('output_B.vsqx'):
                        pass
                    os.remove('output_B.vsqx')
                    with open('output_A.vsqx', 'w', encoding='utf-8') as f:
                        f.write(text)
                except FileNotFoundError:
                    with open('output_A.vsqx', 'w', encoding='utf-8') as f:
                        f.write(text)
                finally:
                    os.startfile('output_A.vsqx')
        else:
            with open(address + 'output.vsqx', 'w', encoding='utf-8') as f:
                f.write(text)

    def get_py(self, text=''):  # api
        if text:
            self.load(text, _vocalized=False)
        py_list = []
        normalized_text_list = []
        for dic in self.mark_list:
            normalized_text_list.append(dic['par'][4])
            if dic['py'][0] != 'sil':
                py_list.append(dic['py'][0]+dic['py'][1])
            else:
                py_list.append('sil')
        out_text = ''.join(normalized_text_list)
        return py_list, out_text

    def get_result(self, text='', speed=1.0, vsqx=True, pyc=None, pbs=16):  # 返回：'py': list, 'pit': list, 'time': list(start, dur)
        if pyc is None:
            pyc = []
        if text:
            self.load(text, speed=speed, pyc=pyc)
        if vsqx:
            ms_per_dot = reliance.global_vars.ms_per_dot
        else:
            ms_per_dot = 1
        ...
        for i, note in enumerate(self.note_list):
            for pit in note[3]:
                p = pit[1]
                location = round((pit[0]) / ms_per_dot)
                value = p / 8190 * pbs
                try:
                    if pits[-1][0] == location:  # 变换后有可能重复，去重
                        value = round((pits[-1][1]+value)/2)
                        del pits[-1]
                except IndexError:
                    pass
                pits.append((location, value))
            for intns in note[4]:
                location = round(intns[0] / ms_per_dot)
                ivalue = round(intns[1])
                try:
                    if intnses[-1][0] == location:  # 变换后有可能重复
                        ivalue = round((intnses[-1][1] + ivalue) / 2)
                        del intnses[-1]
                except IndexError:
                    pass
                intnses.append((location, ivalue))
            if self.mark_list[i]['py'][0] != 'sil':
                pys.append(self.mark_list[i]['py'][0] + self.mark_list[i]['py'][1])
                times.append((round(note[1] / ms_per_dot), round(note[2] / ms_per_dot) - 1))
            else:
                pys.append('sil')
            chrs.append(self.mark_list[i]['par'][4])
        return {'py': pys, 'pit': pits, 'timing': times, 'chr': chrs}  # timing:(start, during)

    def get_pits_by_timing(self, speed=1.0, vsqx=True):
        # 需要每次分析时同时产生字timing和辅音timing，在设置中修改输出模式，而不是在分析时决定
        # 可以只补充其没有值的部分
